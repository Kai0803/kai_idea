...
def close_all_position(trad_type,symbol):
    if trad_type == 'Buy':
        trad_type = 'Sell'
    else:
        trad_type = 'Buy'
...
def trading_strategy(trad_type,symbol):
    while True:
        ...
        if size == 0:
            ...
            place_order(trad_type, min_qty,symbol)
            input_db_trading_data(symbol)
        else:
...
def input_db_trading_data(symbol_temp):
    conn = connect_to_db()
    positions = session.get_positions(category="linear", symbol = symbol_temp)
    ...
    if conn:
        ...
        conn.close()
def get_real_time_position(symbol):
    try:
        positions = session.get_positions(category="linear", symbol = symbol)
        position = positions['result']['list'][0]
        return {
...
if __name__ == "__main__":
   ...
   # trading_strategy(trad_types[0],symbol)
   result = get_real_time_position('ETHUSDT')
   print(result)
   get_position()
